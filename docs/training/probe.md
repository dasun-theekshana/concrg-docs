...
```typescript
import { ProbeOrchestrator } from '@crg/connector-core';

const probe = new ProbeOrchestrator(messageBus, config);
await probe.start();
```

Listen for events via the MessageBus:

```typescript
messageBus.on('probe:progress', (data) => {
  console.log(`Analyzing: ${data.route} (${data.pagesComplete}/${data.totalPages})`);
});

messageBus.on('probe:completed', (data) => {
  console.log(`Done! ${data.triplesExtracted} triples from ${data.pagesDiscovered} pages`);
});
```
...
|---|---|---|
| `probe:started` | `{ appId, timestamp }` | Probe begins |
| `probe:progress` | `{ route, pagesComplete, totalPages }` | Page analyzed |
| `probe:snapshot` | `{ route, elements, screenshot }` | Raw snapshot captured |
| `probe:metrics` | `{ triplesCount, pagesCount }` | Running counts |
| `probe:completed` | `{ pagesDiscovered, triplesExtracted }` | Probe finished |
| `probe:error` | `{ route, error }` | Error on specific page |
